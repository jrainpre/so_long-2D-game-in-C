...
/*   Updated: 2022/11/10 15:21:39 by jrainpre         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_graphics(t_map *map)
{
	...
	if (!map->graphics.wall || !map->graphics.wall || !map->graphics.wall \
	|| !map->graphics.player || !map->graphics.exit || !map->graphics.collec)
	{
		mlx_destroy_window(map->win.mlx, map->win.win);
		error_msg_map("Render failed. Check files.", map);
	}
}
